...
sub print_usage() {
  print STDERR
   ...
   "  --mpi-jobs <mpi-jobs> (default: $mpi_jobs)\n" .
   "  --max-jobs-run <num-jobs>\n" .
   "  --gpu <0|1> (default: $gpu)\n" .
   "  --runq <cpuq|gpuq|3gpuq|4gpuq> (default: $runq)\n";
...
default gpu=0
option gpu=0
option gpu=1 --gres=gpu:1
option gpu=4 --gres=gpu:4 --cpus-per-task 4 # this has to be figured out
option gpu=8 --gres=gpu:8 --cpus-per-task 8 --exclusive
# note: the --max-jobs-run option is supported as a special case
# by slurm.pl and you do not have to handle it in the config file.
EOF
...
# if ( $gpu > 0 && $runq eq "cpuq" ) {
#  print STDERR "$0: you are trying to run a gpu job on cpu queue\n";
#  exit(1);
# }

if ( $gpu != 0 && $gpu != 1 && $gpu != 4 && $gpu != 8) {
  print STDERR "$0: only support using more than 1 gpu or 4 gpu, but you use ($gpu)\n";
  exit(1);
}
...
my $queue_logfile_submit = "${queue_logfile}.submit";
$qsub_cmd .= " $qsub_opts -e ${queue_logfile} -o ${queue_logfile} $queue_array_opt $queue_scriptfile > $queue_logfile_submit 2>&1";
print Q "# $qsub_cmd\n";
...
if ($ret != 0) {
  if ($sync && $ret == 256) { # this is the exit status when a job failed (bad exit status)
    ...
    print STDERR "queue log file is $queue_logfile_submit, command was $qsub_cmd\n";
    print STDERR `tail $queue_logfile_submit`;
  }
  exit(1);
}
...
if (! $sync) { # We're not submitting with -sync y, so we
  ...
  { # Get the SLURM job-id from the log file in q/
    open(L, "<$queue_logfile_submit") || die "Error opening log file $queue_logfile_submit";
    undef $sge_job_id;
    while (<L>) {
      if (m/Submitted batch job (\d+)/) {
        if (defined $sge_job_id) {
          die "Error: your job was submitted more than once (see $queue_logfile_submit)";
        } else {
          $sge_job_id = $1;
        }
      }
    ...
    close(L);
    if (!defined $sge_job_id) {
      die "Error: log file $queue_logfile_submit does not specify the SLURM job-id.";
    }
  }
  ...
  foreach my $f (@syncfiles) {
    ...
    while (! -f $f) {
      ...
      if (($check_sge_job_ctr++ % 10) == 0) { # Don't run qstat too often, avoid stress on SGE.
        ...
        my ($output, $status) = exec_command("sjobexitmod -l $sge_job_id");
        if ( $output =~ "FAIL" || $output =~ "CANCEL" ){
            print "Some jobs failed, suppose to be shut down \n";
            print $output;
            caught_signal();
        }
