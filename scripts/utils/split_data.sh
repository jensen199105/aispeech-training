...
for n in `seq $numsplit`; do
   ...
   sweep3s="$sweep3s $data/split$numsplit/$n/sweep3"
   sweep2s="$sweep2s $data/split$numsplit/$n/sweep2"
done
...
[ -f $data/sweep3 ] && utils/split_scp.pl $utt2spk_opt $data/sweep3 $sweep3s
[ -f $data/sweep2 ] && utils/split_scp.pl $utt2spk_opt $data/sweep2 $sweep2s
